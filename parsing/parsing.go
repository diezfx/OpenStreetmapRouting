...
func (d *DataHandlerStep2) ReadNode(n gosmparse.Node) {
	...
	if nodeID, ok := d.Graph.NodeIDs[n.ID]; ok == true {
		...
		if nodeID == -1 {
			...
			node := data.Node{ID: int64(len(d.Graph.Nodes)), Lat: n.Lat, Lon: n.Lon}
...
// TODO
// complocated version with channels
// when an edge is read and the corresponding nodes send node ids to channel
// -> no parallelization in that thread

// channel creates both at the same time hashmap with newId and slice with real objects
